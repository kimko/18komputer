...
- **Features:**
  ...
  - **Setting a Share Price:** The price picker matches the shape of the title's real stock market. Most titles, 1830 among them, use a grid, and the popup draws it with the company's square highlighted, the colour zones shaded and the par squares outlined. Four arrows move the company one square at a time and follow the game's own rules: running off the end of a row moves it up a row instead, a left move at the left edge becomes a move down, and a downward move stops dead at a ledge where there is no square below. An arrow that cannot move anything is greyed out, so a ledge is visible rather than an unresponsive button. The popup stays open while you press, so selling three shares is three presses of down. Titles with a flat market keep the plain list of prices with left and right arrows, and tapping any square or price directly sets it and closes. The square a company is standing on is saved with the game, so two squares sharing the same money value are never confused with each other, and reopening the game puts the company back exactly where it was.
  ...
  - **Analysis:** A fourth tab works out what the recorded rounds were worth to hold, rather than just what the table says now. For each company it splits what one share earned into the dividends it collected and what its price did, and shows the two as one bar so the length is the total and a price that fell reads as a loss to the left of zero. It works the price out backwards: each operating round earns a move by the title's own rules (paying moves right, a round recorded as zero means withholding and moves left, and no shares left in the bank means the company was sold out and moved up), so replaying those moves from every square on the chart finds where the price must have stood when the last share round closed. A table under the chart carries every figure, the same split is shown per player, and a second small chart says how much of each player's cash the recorded rounds actually account for. Under it all, a plain sentence per company and per player says the same thing in words. Where several starting squares would produce the recorded price the figure is marked approximate and the cautious one is used; where no square would, it says so rather than inventing a number. That gap is usually shares traded during the share round, which move a price and are not recorded anywhere.
...
- **Two supported printers:**
  ...
  - **GOOJPRT PT-210**, a 58mm receipt printer. Sends plain text using the printer's own built-in font, 32 characters per line, so a whole receipt is about 270 bytes and prints as one continuous strip. The company name is bold and double height, the total is pushed to the right edge, and long routes wrap on a `+` so no stop value is ever cut in half. Each train line and the total print double height so they can be read at a glance; the printer has no half sizes, and growing the width instead would cut the line to 21 characters and split routes across two of them. Every slip ends with a blank line and then the app version, centred. It is in the normal typeface because this printer accepts the command to select its second, smaller font and then prints nothing, having no glyphs for it.
- **Features:**
