...
- **Features:**
  - **The payout is on the slip (PT-210).** Under the total the receipt records the company type, whether it paid full or half, the dividend per share, and the money going to the treasury, so the paper is a complete record of the round. The figures come from the same calculation as the on-screen table, so the two cannot disagree. The label printer is left as it was, because every extra line there costs another sticker.
