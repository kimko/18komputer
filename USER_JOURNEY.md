...
- **Features:**
  ...
  - **Share:** A "Share" button at the top-right saves the current game to a Google Sheet and copies a link to the clipboard. The link carries only the game's id, so it is short enough to fit anywhere, and the game itself is fetched from the sheet when someone opens it. Sharing the same game again updates its row rather than adding another. If the sheet cannot be reached the button says why and copies nothing, because a link to a game that was never saved would only fail later. Anyone who opens the link has the game imported and opened on their device; if they already have that game, they are asked which copy to keep before anything is overwritten.
